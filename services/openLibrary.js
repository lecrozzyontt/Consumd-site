...
export async function fetchTrendingBooks() {
  return cached('ol:trending', async () => {
    try {
      ...
      return data.works?.map(formatBook).filter(b => b.cover_url) || [];
    } catch { return []; }
  }, { ttl: TRENDING_TTL });
}
...
export async function fetchTopRatedBooks() {
  return cached('ol:top_rated', async () => {
    try {
      ...
      const data = await res.json();
      return data.docs?.map(formatBook).filter(b => b.cover_url) || [];
    } catch { return []; }
  }, { ttl: TRENDING_TTL });
}
...
export async function searchBooks(query) {
  ...
  return cached(`ol:search:${query.toLowerCase()}`, async () => {
    try {
      const res  = await fetch(`${BASE}/search.json?q=${encodeURIComponent(query)}&limit=20`);
      const data = await res.json();
      return data.docs?.map(formatBook).filter(b => b.cover_url) || [];
    } catch { return []; }
  }, { ttl: SEARCH_TTL });
    ...
    try {
      ...
      if (work.authors?.length) {
        const authorFetches = work.authors.slice(0, 3).map(a => {
          ...
          return key ? fetch(`${BASE}${key}.json`).then(r => r.json()).catch(() => null) : null;
        });
        const authorData = await Promise.all(authorFetches);
        authors = authorData.filter(Boolean).map(a => ({
          name:    a.name || '',
